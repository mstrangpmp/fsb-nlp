"""
==============================================================================
02 - Bách Nhân Baseline: TextRank for Vietnamese Real-Estate Summarization
==============================================================================
Pipeline:
  1. Pull newest data from Google Sheets (public CSV export)
  2. Run Vietnamese TextRank on raw descriptions
  ...
  6. Save:
       - Nhan_Baseline_predictions.json
       - Nhan_Baseline_performance.json

Usage:
    pip install underthesea scikit-learn rouge-score numpy requests tqdm
    python baseline_textrank.py
==============================================================================
"""
...
# Sentence boundary: handled inside _split_sentences / underthesea
...
def textrank_summarize(text: str, n_sentences: int = 3) -> str:
    """
    Vietnamese TextRank extractive summarization.
    Returns top-N sentences in original order, skipping boilerplate.
    """
...
def extract_specs(text: str) -> dict:
    ...
    # ── Diện tích: "77.7m2", "38.7 m2"
    dt_matches = re.findall(r'(\d+\.?\d*)\s*m2', t)
    dien_tich = float(dt_matches[0]) if dt_matches else None

    # ── Giá tỷ: "27.5 tỷ", "nhỉnh 8 tỷ", "8ty99", "13ty99"
    gia = None
    m = re.search(r'(\d+[,.]?\d*)\s*(?:tỷ|ty)(?:\s*(?:tl|thương lượng))?', t)
    if m:
        gia = float(m.group(1).replace(',', '.'))

    # ── Số tầng: "5 tầng", "4 lầu", "1 trệt 3 lầu" → sum trệt + lầu
    so_tang = None
    # Pattern: "X trệt Y lầu" — limit span to 80 chars to avoid cross-paragraph matches
    ...
    # ── Chiều ngang: "ngang 4", "4.3m x", "4x", "ngang 5.5m"
    ngang = None
    ...
    # ── Chiều sâu / dài: "dài 13", "sâu 12", "x12", "x 15m"
    sau = None
    ...
    # ── Phân loại hẻm: prioritize alley types over "Mặt tiền" when they coexist
    ...
    # ── Quận: "quận 3", "quận 10", "bình thạnh", "phú nhuận"
    quan = None
    ...
    # ── Phường: "phường 3", "phường nhiêu lộc", "phường bàn cờ"
    phuong = None
    m = re.search(r'(?:phường|p\.)\s*([\w\s]+?)(?:\s+(?:quận|hcm|tp|,|\.|$))', t)
    if m:
        phuong = f"Phường {m.group(1).strip().title()}"

    # ── Tên đường (from original case-preserved text)
...
def _extract_street_name(text: str) -> Optional[str]:
    """
    Try to find the street name from the first line of the raw listing
    (agents usually put 'Tên đường DT SốTầng Ngang Sâu Giá …' as the header).
    """
    first_line = text.strip().splitlines()[0] if text.strip() else ""
    # Pattern: first capitalised word-group before a number
    m = re.match(
...
def generate_title(specs: dict) -> str:
    """
    Format: Đường Quận - DT (ngang x sâu) - Hem_type - Số tầng - Giá Tỷ
    Example: Lê Văn Sỹ Quận 3 - 46m2 (4.6x10) - Hẻm xe hơi - 4 tầng - 12.8 Tỷ
    """
    parts = []
...
def compute_rouge(predictions: list) -> Optional[dict]:
    """Compute average ROUGE-L F1 on rows that have ground-truth descriptions."""
    if not HAS_ROUGE:
        return None
...
def run_pipeline():
    ...
    print(f"\n🚀 Running TextRank on {len(rows)} listings …")
    for row in tqdm(rows, desc="TextRank"):
        raw = row["raw_input"]
        sid = row["id"]

        t0 = time.perf_counter()

        description = textrank_summarize(raw, N_SUMMARY_SENTENCES)
        ...
        predictions.append({
            ...
            # Ground truth kept alongside for evaluation (not part of DoD output)
    ...
    # Strip GT fields from the DoD-facing output
    dod_predictions = []
    ...
    pred_path = OUTPUT_DIR / "Nhan_TextRank_predictions.json"
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump(dod_predictions, f, ensure_ascii=False, indent=2)
    print(f"\n💾 Saved: {pred_path}  ({len(dod_predictions)} records)")

    # Save a copy as Nhan_Baseline_predictions.json to support Leader's aggregate script
    baseline_pred_path = OUTPUT_DIR / "Nhan_Baseline_predictions.json"
    with open(baseline_pred_path, "w", encoding="utf-8") as f:
        json.dump(dod_predictions, f, ensure_ascii=False, indent=2)
    print(f"💾 Saved copy: {baseline_pred_path}")

    # ── Step 5: Save performance ──────────────────────────────────────────────
    performance = {
        "model_name":                 "TextRank (Vietnamese TF-IDF + PageRank)",
        "algorithm":                  "Graph-based Extractive Summarization",
    ...
    perf_path = OUTPUT_DIR / "Nhan_TextRank_performance.json"
    with open(perf_path, "w", encoding="utf-8") as f:
        json.dump(performance, f, ensure_ascii=False, indent=2)
    print(f"💾 Saved: {perf_path}")

    # Save a copy as Nhan_Baseline_performance.json to support Leader's aggregate script
    baseline_perf_path = OUTPUT_DIR / "Nhan_Baseline_performance.json"
    with open(baseline_perf_path, "w", encoding="utf-8") as f:
        json.dump(performance, f, ensure_ascii=False, indent=2)
    print(f"💾 Saved copy: {baseline_perf_path}")
