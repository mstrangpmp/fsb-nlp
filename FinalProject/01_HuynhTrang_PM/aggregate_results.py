...
def aggregate_and_evaluate():
    ...
    for model_name, file_prefix in MEMBERS.items():
        # Kiểm tra file predictions ở thư mục hiện tại hoặc ở thư mục riêng của từng bạn
        # Nhắm tới việc Trang có thể copy các file về đây hoặc chỉ định đường dẫn
        ...
        paths_to_try_pred = [pred_file, f"../{file_prefix.replace('HuuHieu', '06_HuuHieu').replace('Nhan_Baseline', '02_BachNhan').replace('Quan_GPT', '03_MinhQuan').replace('Hoc_Gemini', '04_BaHoc').replace('MaiAnh_ViT5', '05_MaiAnh')}/{pred_file}"]
        paths_to_try_perf = [perf_file, f"../{file_prefix.replace('HuuHieu', '06_HuuHieu').replace('Nhan_Baseline', '02_BachNhan').replace('Quan_GPT', '03_MinhQuan').replace('Hoc_Gemini', '04_BaHoc').replace('MaiAnh_ViT5', '05_MaiAnh')}/{perf_file}"]
